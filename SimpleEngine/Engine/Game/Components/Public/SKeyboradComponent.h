...
namespace SEngine{
	...
	class SKeyboardComponent final : public SGameComponent
	{
		SRTTI_DECLARATIONS(SKeyboardComponent, SGameComponent)

	public:
		static DirectX::Keyboard* Keyboard();

		SKeyboardComponent(SGame& game);
		SKeyboardComponent(const SKeyboardComponent&) = delete;
		SKeyboardComponent(SKeyboardComponent&&) = default;
		SKeyboardComponent& operator=(const SKeyboardComponent&) = delete;
		SKeyboardComponent& operator=(SKeyboardComponent&&) = default;
		~SKeyboardComponent() = default;
	...
	/*
	template <typename T>
	void UpdateValueWithKeyboard(const SKeyboardComponent& keyboard, const Keys increaseKey, const Keys decreaseKey, T& value, const T& delta, std::function<void(const T&)> updateFunc = nullptr, const T& minValue = std::numeric_limits<T>::lowest(), const T& maxValue = std::numeric_limits<T>::max())
	{
	...
	template <typename T>
	void IncrementEnumValue(const SKeyboardComponent& keyboard, const Keys increaseKey, T& value, std::function<void(T)> updateFunc, T maxValue, T minValue = T(0), bool wrap = true)
	{

...
class SCamera : public SGameComponent
{
	SRTTI_DECLARATIONS(SCamera, SGameComponent)

public:
	explicit SCamera(SGame& game, float nearPlaneDistance = DefaultNearPlaneDistance, float farPlaneDistance = DefaultFarPlaneDistance);
	SCamera(const SCamera&) = default;
	SCamera& operator=(const SCamera&) = default;
	SCamera(SCamera&&) = default;
	SCamera& operator=(SCamera&&) = default;
